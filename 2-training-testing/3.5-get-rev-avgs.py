...
"""Caluculate the reversed average mse, mae and hitrate for each model,
...
for model in os.listdir(INPUT_DIR):
    ...
    for dataset_type in os.listdir(model_path):
        ...
        for target in os.listdir(dataset_path):
            ...
            for metric in ('mse','mae','hitrate'):
                ...
                # Load data
                df_train = pd.read_csv(target_path / f"rev_train_{metric}_results.csv")
                df_test = pd.read_csv(target_path / f"rev_test_{metric}_results.csv")
                ...
                print(target_path / f"rev_avg_{metric}.csv")
    ...
    df.to_csv(model_path / 'rev_mae_results.csv', index=False)
    
    df = pd.DataFrame(list(hitrate_results.items()), columns=['type', 'avg_hitrate'])
    df.to_csv(model_path / 'rev_hitrate_results.csv', index=False)

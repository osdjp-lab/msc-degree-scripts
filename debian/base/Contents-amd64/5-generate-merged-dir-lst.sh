...
# Libraries
# /{lib,lib32,lib64,libx32}; /usr/{lib,lib32,libx32}

LIB="$MERGED_LISTINGS_DIR/lib"

lib="$SPLIT_LISTINGS_DIR/lib"
cat "${lib}.lst" "${lib}32.lst" "${lib}64.lst" "${lib}x32.lst" > "$LIB"

lib="$SPLIT_LISTINGS_DIR/usr/lib"
cat "${lib}.lst" "${lib}32.lst" "${lib}x32.lst" \
    | sed -e 's/^/usr\//' >> "$LIB"
